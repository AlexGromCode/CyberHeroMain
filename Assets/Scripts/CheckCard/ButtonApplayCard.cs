...
public class ButtonApplayCard : MonoBehaviour
{
    ...
    private void Awake()
    {
        _applayCard.onClick.AddListener(CheckResult);
    ...
    public void CheckResult()
    {
        ...
        if (_checkCard != null && GM.IsPlayingRoomCheckCard)
        {
            ...
            else
            {
                ...
                {
                    _dialogue.ShowLineExtra(indexExtrsLine: 0, scene: 1, indexImage: 0, indexVoice: 0);
                    GM.Room4Win = true;
                    PlayerMain.CountKeys++;
                }
                else
                    _dialogue.ShowLineExtra(indexExtrsLine: 1, scene: 8, indexImage: 1, indexVoice: 1);
            }
        }
    }
}